...
class ImageButton : protected CustomWindow
{
	private:
		...
		ImageWindow icon;
	public:
		ImageButton(CustomWindow& Parent, const std::string& title, const HANDLE icon, int x, int y, int width, int height , std::function<void(ImageButton&)> OnClick)
			:
		...
		{
			mouse.OnLeftRelease = [this , OnClick](auto& mouse)
			{
				OnClick(*this);
			};
		}
